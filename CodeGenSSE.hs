...
---- Portability :  GHC 7.6
----
---- CodeGenSSE  :  Code generator for x86 SSE2.
----
-------------------------------------------------------------------------------
module CodeGenSSE where
...
printTree :: (CodeGenSSE a) => a -> String
-- printTree tree = headerString ++ (render $ prt tree)
printTree tree = "#include \"mudaintrin_sse.h\"\n\n" ++(render $ prt tree)
...
instance CodeGenSSE Char where
...
class CodeGenSSE a where
  prt :: a -> Doc
  prtList :: [a] -> Doc
  prtList = concatD . map prt


instance CodeGenSSE a => CodeGenSSE [a] where
  prt = prtList
...
instance CodeGenSSE IR.Exp where
  prt e = case e of
    ...
    IR.EMul   sym e0 e1
      ...
      | (tyOfExp e0) == T.DVec -> prtBinOpN  "_mm_mul_pd"    sym e0 e1 (vLen sym)
    ...
    --
    -- For double precision case, there's no approximate division in SSE
...
-}
    ...
    --
    IR.EMin sym exps
      | (tyOfExp $ exps !! 0) == T.Vec  -> prtBinOpN "_mm_min_ps" sym (exps !! 0) (exps !! 1) (vLen sym)
      | (tyOfExp $ exps !! 0) == T.DVec -> prtBinOpN "_mm_min_pd" sym (exps !! 0) (exps !! 1) (vLen sym)
    ...
    --
    IR.EMax sym exps
      | (tyOfExp $ exps !! 0) == T.Vec  -> prtBinOpN "_mm_max_ps" sym (exps !! 0) (exps !! 1) (vLen sym)
      | (tyOfExp $ exps !! 0) == T.DVec -> prtBinOpN "_mm_max_pd" sym (exps !! 0) (exps !! 1) (vLen sym)
      | otherwise                       -> error $ "TODO" ++ show e
    ...
    IR.EFieldSelect sym id exp
      ...
      where
        ...
        structVarNameFromExp e = case e of
          ...
          _                                   -> error $ "[CodeGenSSE] TODO " ++ show e
...
--
instance CodeGenSSE IR.MDec where
  prt p = case p of
    IR.MDecl ty id structId -> case (getTypeName ty) of
      ...
      _      -> error "[CodeGenSSE] IR.MDecl: TODO"
...
instance CodeGenSSE IR.Struc where
  prt s = case s of
...
instance CodeGenSSE IR.Func where
  prt f = case f of
...
--
instance CodeGenSSE IR.FormalDec where
  prt e = case e of
...
headerString = unlines
  ...
  , "}"
  , ""
  , "MUDA_STATIC MUDA_ALWAYS_INLINE __m128d muda_sel_pd( const __m128d a, const __m128d b, const __m128d mask )"
  , "{"
  , "#ifdef __SSE41__"
  , "    return _mm_blend_pd(a, b, mask);"
  , "#else"
  , "    const __m128d tmp0 = _mm_and_pd( b, mask );"
  , "    const __m128d tmp1 = _mm_andnot_pd( mask, a);"
  , "    return _mm_or_pd( tmp1, tmp0 );"
  , "#endif"
  , "}"
  , ""
  , "MUDA_STATIC MUDA_ALWAYS_INLINE int muda_gather_pd( const __m128d a, const __m128d b )"
  , "{"
  ...
  , "}"
  , ""
  , "MUDA_STATIC MUDA_ALWAYS_INLINE __m128d muda_divapprox_pd( const __m128d a, const __m128d x )"
  , "{"
  , "    __m128d b = _mm_cvtps_pd(_mm_rcp_ps(_mm_cvtpd_ps(x))); "
  , "    b = _mm_sub_pd(_mm_add_pd(b, b), _mm_mul_pd(_mm_mul_pd(x, b), b)); "
  , "    b = _mm_sub_pd(_mm_add_pd(b, b), _mm_mul_pd(_mm_mul_pd(x, b), b)); "
  , "    return _mm_mul_pd(a, b); // = a * (1.0 / x)"
  , "}"
  , ""
...
prtSym sym = case (Sym.getKind sym) of
  ...
  _                      -> error $ "[CodeGenSSE] prtSym: TODO: " ++ show sym 
...
prtArraySym sym = case (Sym.getKind sym) of
  ...
  _                      -> error $ "[CodeGenSSE] prtArraySym: TODO: " ++ show sym 
...
prtAccessFormalSym sym = case (isPointer $ Sym.getQuals sym, isScalar $ Sym.getType sym) of
  ...
  where
    ...
    nameWithTypeCast sym = case Sym.getType sym of
      ...
      T.DVec     -> "((__m128d *)(" ++ (Sym.getName sym) ++ "))"
      T.F        -> (Sym.getName sym)
      T.D        -> (Sym.getName sym)
      T.I        -> (Sym.getName sym)
      T.L        -> (Sym.getName sym)
...
prtAccessFormalSymN sym n = case (isPointer $ Sym.getQuals sym, isScalar $ Sym.getType sym) of
  ...
  where
    ...
    nameWithTypeCastN sym n = case Sym.getType sym of
      T.Vec      -> "((__m128 *)("  ++ (Sym.getName sym) ++ "+" ++ show (4 * (n-1)) ++ "))"
      T.IVec     -> "((__m128i *)(" ++ (Sym.getName sym) ++ "+" ++ show (4 * (n-1)) ++ "))"
      T.DVec     -> "((__m128d *)(" ++ (Sym.getName sym) ++ "+" ++ show (2 * (n-1)) ++ "))"
      T.F        -> (Sym.getName sym)
      T.D        -> (Sym.getName sym)
      T.I        -> (Sym.getName sym)
      T.L        -> (Sym.getName sym)
...
prtRefFormalSym sym = case Sym.getType sym of
  ...
  T.IVec     -> docStr $ "_mm_loadu_si128((__m128i const *)(" ++ (Sym.getName sym) ++ "))"
...
prtRefFormalSymN sym n = case Sym.getType sym of
  ...
  T.IVec     -> docStr $ "_mm_loadu_si128((__m128i const *)(" ++ (Sym.getName sym) ++ " + " ++ show (4 * (n-1)) ++ "))"
...
prtReadIdentSymN sym n = case (Sym.getKind sym) of
  Sym.KindVariable       -> prtVarSymN sym n
  Sym.KindFormalVariable -> prtReadFormalSymN sym n 
  _                      -> error $ "[CodeGenSSE] prtAccessIdentSym: TODO: " ++ show sym
...
instance CodeGenSSE IR.DecInitExp where
  prt e = case e of
    IR.DExp sym exp          -> concatD [prt exp]

instance CodeGenSSE IR.DecInit where
  prt d = case d of
...
instance CodeGenSSE IR.Stm where
  prt e = case e of
...
instance CodeGenSSE IR.FuncSpec where
  prt f = case f of
...
instance CodeGenSSE IR.Prog where
  prt p = case p of
...
instance CodeGenSSE IR.Ident where
